...
from .pro import decode_literal_string, parse_list, resolve_scripts_dir, run_swebench_pro_eval
...
class SweDatasetAdapter:
    ...
    def image_name(
        ...
        if env_type in ("docker", "modal"):
            return (f"docker.io/{self.namespace}/{self.image_dataset}{split}.x86_64." f"{image_id}:latest").lower()
        if env_type == "enroot":
            return (f"{self.namespace}+{self.image_dataset}{split}.x86_64." f"{image_id}+latest.sqsh").lower()
...
class SweBenchProDatasetAdapter(SweDatasetAdapter):
    ...
    def setup_environment(
        ...
        env_obj.execute(
            f"cd {workspace_dir} && git reset --hard {base_commit} " f"&& git clean -fd && git checkout {base_commit}"
        )
    ...
    def evaluate(
        ...
        if scripts_dir is None:
            raise ValueError(
                "SWE-Bench Pro eval needs official run scripts. Set " "swebench_pro_scripts_dir or swebench_pro_repo."
            )
