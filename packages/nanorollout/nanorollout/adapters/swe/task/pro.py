...
def resolve_scripts_dir(extra_args: dict[str, Any]) -> Optional[Path]:
    ...
    repo_dir = extra_args.get("swebench_pro_repo") or extra_args.get("pro_repo") or os.environ.get("SWEBENCH_PRO_REPO")
    if repo_dir:
...
def _extract_json(output: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ValueError("SWE-Bench Pro parser output was not JSON: " f"{raw_json[-1000:] or '<empty>'}") from exc


def _tests_by_status(output_json: dict[str, Any], status: str) -> set[str]:
    return {
        str(test.get("name")) for test in output_json.get("tests", []) if str(test.get("status", "")).upper() == status
    }


def _report(instance: dict[str, Any], output_json: dict[str, Any]) -> dict[str, Any]:
    passed = _tests_by_status(output_json, "PASSED")
    fail_to_pass = set(parse_list(instance.get("fail_to_pass") or instance.get("FAIL_TO_PASS")))
    pass_to_pass = set(parse_list(instance.get("pass_to_pass") or instance.get("PASS_TO_PASS")))
    return {
        "FAIL_TO_PASS": {
...
def _build_eval_script(
    ...
    run_command = (
        f"bash {run_script} {shlex.quote(selected_tests)} " f"> {stdout_log} 2> {stderr_log}"
        if selected_tests
    ...
    return "\n".join(
        [
            ...
            "git config --global --add safe.directory " f"{shlex.quote(workspace_dir)}",
        ...
        return (
            {
                ...
                "status_map": {
                    str(test.get("name")): str(test.get("status")) for test in output_json.get("tests", [])
                },
                "report": report,
                "output": output_json,
            },
