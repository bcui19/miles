...
def _modified_files(patch: str) -> list[str]:
    files: list[str] = []
    seen: set[str] = set()
    for path in re.findall(r"^diff --git a/(?:.*?) b/(.*?)$", patch, flags=re.MULTILINE):
        if path not in seen:
            files.append(path)
            seen.add(path)
    return files
...
def _build_report(
    ...
    try:
        from swebench.harness.constants import FAIL_TO_FAIL, FAIL_TO_PASS, PASS_TO_FAIL, PASS_TO_PASS, EvalType
    ...
    report = get_eval_tests_report(
        status_map,
        gold_results,
        calculate_to_fail=bool(gold_results[FAIL_TO_FAIL] or gold_results[PASS_TO_FAIL]),
