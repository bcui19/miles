...
def setup_r2egym_env(env: Any, workspace_dir: str = "/testbed", alt_path: str = "/root") -> None:
    """Mirror the small R2E-Gym container setup NanoRollout needs."""
    ...
    env.execute(f"find {workspace_dir}/.venv/bin -type f -executable -exec ln -sf {{}} {alt_path}/.local/bin/ \\;")
...
def compare_results(
    ...
    for k in parsed:
        ...
        if k not in expected:
            ...
            mismatches.append({"key": k, "parsed": parsed[k], "expected": expected[k]})
...
def get_r2egym_eval_report(
    ...
    *,
    ...
    try:
        ...
        logger.error("[%s] Failed to parse expected_output_json: %s", instance_id, exc)
        return {
...
def run_r2egym_eval(
    ...
    *,
    ...
    try:
        logger.info("[%s] Running R2E-Gym tests: bash %s", instance_id, test_script_path)
