...
import tomllib
import yaml
from nanorollout.runner import build_reward_payload as _shared_build_reward_payload
...
def resolve_tb_repo_dir(
    ...
    *,
    ...
    cache_root = Path(os.environ.get("TB_REPO_CACHE_DIR", "~/.cache/nanorollout/terminalbench")).expanduser()
...
def ensure_tb_image(instance: dict[str, Any], env_type: str = "docker") -> str:
    ...
    if not dockerfile_dir:
        raise ValueError(f"Instance {instance.get('instance_id')} has no docker_image and no dockerfile_dir")

    if env_type in ("modal", "daytona"):
        instance["docker_image"] = dockerfile_dir
        return dockerfile_dir
    if env_type not in ("docker", "enroot"):
        raise ValueError(f"Building from Dockerfile is not supported for env_type={env_type}")
...
def resolve_tb_timeouts(
    ...
    agent_timeout = (
        task_agent * multiplier if global_agent_cap == -1 else min(task_agent, global_agent_cap) * multiplier
    )

    global_eval_cap = extra_args.get("eval_timeout", float("inf"))
    task_eval = instance.get("eval_timeout_sec", 600.0)
    eval_timeout = task_eval * multiplier if global_eval_cap == -1 else min(task_eval, global_eval_cap) * multiplier
