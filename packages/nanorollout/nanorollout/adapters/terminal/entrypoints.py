...
def _run_terminus2_agent(agent: Any, task: TaskSpec, env_obj: Any) -> Any:
    del env_obj
    return agent.run(task.instruction, total_timeout=task.environment.get("agent_timeout"))
...
def _run_installed_agent_result(
    ...
    return agent.run(
        task.instruction,
        env_obj,
        timeout_sec=(int(task.environment["agent_timeout"]) if task.environment.get("agent_timeout") else None),
    )
...
run_tb_claude_code = _make_terminal_runner(_installed_spec("claude-code", "run_tb_claude_code"))
run_tb_qwen_code = _make_terminal_runner(_installed_spec("qwen-code", "run_tb_qwen_code"))
