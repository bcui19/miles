...
_PYTEST_STATUS_RE = re.compile(r"^(PASSED|FAILED|ERROR|SKIPPED|XFAIL|XPASS)\s+(\S+)", re.MULTILINE)
    ...
    try:
        ...
        if not run_tests_script or not Path(run_tests_script).is_file():
            raise FileNotFoundError(
                f"Terminal-Bench v1 task {instance_id} missing run-tests.sh " f"(run_tests_script={run_tests_script})"
            )
        if tests_dir:
        ...
        logger.info("[%s] Uploading run-tests.sh from %s", instance_id, run_tests_script)
        env_obj.upload_file(run_tests_script, "/run-tests.sh")
        env_obj.execute("mkdir -p /logs/verifier")

        logger.info("[%s] Executing /run-tests.sh (timeout=%ss)", instance_id, timeout)
...
def run_tb_eval(
    ...
    try:
        ...
        # Run the test script
        logger.info("[%s] Executing /tests/test.sh (timeout=%ss)", instance_id, timeout)
        eval_result = env_obj.execute(
