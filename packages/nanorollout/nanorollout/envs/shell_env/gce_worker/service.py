...
from .docker_runtime import STREAM_EXIT, STREAM_STDERR, STREAM_STDOUT, DockerRuntime
...
class WorkerService(worker_pb2_grpc.WorkerServiceServicer):
    ...
    async def StopContainer(
        ...
        loop = asyncio.get_running_loop()
        try:
            await loop.run_in_executor(None, self._runtime.stop_container, request.container_id)
        except Exception as exc:
        ...
        def producer() -> None:
            try:
                for kind, payload, exit_code in self._runtime.exec_streaming(
                    ...
                    asyncio.run_coroutine_threadsafe(queue.put((kind, payload, exit_code)), loop).result()
                ...
                asyncio.run_coroutine_threadsafe(queue.put((-1, str(exc).encode("utf-8"), 1)), loop).result()
            finally:
    ...
    async def ListDir(
        ...
        try:
            ...
            raise
        proto_entries = [worker_pb2.DirEntry(name=e.name, is_dir=e.is_dir, size=e.size) for e in entries]
