...
class EnrootEnvironment(ShellEnvironment):
    ...
    def _resolve_image_path(self) -> None:
        ...
        for cache_dir in cache_paths:
            for name in candidate_names:
                ...
                if candidate.exists():
                    self.logger.info("Resolved image %s to %s", self.config.image, candidate)
                    self.config.image = str(candidate.resolve())
                    return

        self.logger.error("Image %s not found in any cache directory", self.config.image)
        raise FileNotFoundError(f"Image {self.config.image} not found in any cache directory")
    ...
    def start(self) -> None:
        ...
        self.logger.info("Created container %s: %s", self.container_name, result.stdout.strip())
        ...
        try:
            ...
            if not pid_str or not pid_str.isdigit():
                ...
                raise RuntimeError("Failed to get PID from container. " f"stdout: '{pid_str}', stderr: '{stderr_out}'")
    ...
    def _terminate_process_group(self, process: subprocess.Popen[str], *, term_timeout_s: float = 5.0) -> None:
    ...
    def _start_orphan_reaper(self) -> None:
        ...
        self._stop_orphan_reaper()
        if self.container_name is None or self.container_pid is None or self.container_process is None:
            return
    ...
    def _run_subprocess(
        ...
        *,
        ...
        try:
            ...
            output = f"{partial_output.rstrip()}\n{timeout_msg}" if partial_output else timeout_msg
    ...
    def write_file(self, path: str, content: str) -> ExecutionResult:
        ...
        cmd = f"cat > {repr(path)} << 'NANOROLLOUT_EOF'\n" f"{content}\nNANOROLLOUT_EOF"
