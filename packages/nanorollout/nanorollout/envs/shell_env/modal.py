...
class ModalEnvironment(ShellEnvironment):
    ...
    def __init__(
        ...
        invalid_env_names = sorted(name for name in self.config.env if not _ENV_VAR_NAME_PATTERN.match(name))
        if invalid_env_names:
            raise ValueError("Invalid environment variable names: " + ", ".join(invalid_env_names))
    ...
    async def _start_async(self) -> None:
        ...
        if self.config.secrets:
            sandbox_kwargs["secrets"] = [Secret.from_name(secret_name) for secret_name in self.config.secrets]
        if self.config.volumes:
            sandbox_kwargs["volumes"] = {
                mount_path: Volume.from_name(volume_name) for mount_path, volume_name in self.config.volumes.items()
            }
        ...
        if exit_code != 0:
            raise RuntimeError(f"Failed to create workspace {self.workspace_dir}: {stderr}")
    ...
    def is_running(self) -> bool:
        if self._sandbox is None:
            return False
        try:
            _, _, exit_code = self._run_async(self._exec_process_async("true", timeout_sec=5))
            return exit_code == 0
        except Exception:
            return False
    ...
    def _run_async(coro: Any) -> Any:
        ...
        raise RuntimeError(
            "ModalEnvironment provides synchronous APIs and cannot run inside an " "already running event loop."
        )
