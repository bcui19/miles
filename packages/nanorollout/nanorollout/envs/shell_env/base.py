...
DEFAULT_TRUNCATE_NOTICE_WITH_PERSIST = "<response clipped; full output saved to {file_path} at line {line_num}>"
...
def maybe_truncate(
    ...
    return content[:head_chars] + final_notice + (content[-tail_chars:] if tail_chars > 0 else "")
...
class ShellEnvironment(ABC):
    ...
    def execute_tool(self, tool_name: str, **kwargs) -> "ToolResult":
        ...
        if handler is None:
            if tool_logger.isEnabledFor(logging.WARNING):
                tool_logger.warning("%stool_missing name=%s", log_prefix, tool_name)
            return ToolResult(
        ...
        if tool_logger.isEnabledFor(logging.INFO):
            ...
            metadata_for_log = (
                {key: value for key, value in metadata.items() if key != "log_output"} if metadata else metadata
            )
            tool_logger.info(
    ...
    def _tool_execute_bash(
        ...
        **kwargs,
        ...
        if result.exit_code != -1:
            suffix_lines.append(f"[Command finished with exit code {result.exit_code}]")
    ...
    def _editor_validate_path(self, command: str, path: str) -> Optional["ToolResult"]:
        ...
        if command == "create":
            if self._editor_path_exists(path):
                return ToolResult(
                    output=(f"File already exists at: {path}. Cannot overwrite files using " "command `create`."),
                    success=False,
                )
            return None
        ...
        if command != "view" and self.is_directory(path):
            return ToolResult(
                output=(f"The path {path} is a directory and only the `view` command can " "be used on directories."),
                success=False,
            )
        return None
    ...
    def _editor_make_output(self, snippet_content: str, snippet_description: str, start_line: int = 1) -> str:
    ...
    def _editor_view(self, path: str, view_range: Optional[list[int]] = None) -> "ToolResult":
        ...
        if self.is_directory(path):
            ...
            path_quoted = shlex.quote(path)
            hidden_result = self.execute(f"find -L {path_quoted} -mindepth 1 -maxdepth 1 -name '.*'")
            hidden_count = len(hidden_result.output.strip().splitlines()) if hidden_result.output.strip() else 0

            hidden1 = shlex.quote(f"{path}/.*")
            hidden2 = shlex.quote(f"{path}/*/.*")
            list_cmd = f"find -L {path_quoted} -maxdepth 2 -not \\( -path {hidden1} -o " f"-path {hidden2} \\) | sort"
            list_result = self.execute(list_cmd)
            if list_result.exit_code != 0:
                return ToolResult(output=list_result.output, success=False)

            paths = list_result.output.strip().splitlines() if list_result.output.strip() else []
            formatted_paths = []
            for entry in paths:
    ...
    def _editor_str_replace(self, path: str, old_str: Optional[str], new_str: Optional[str]) -> "ToolResult":
        ...
        if new_str == old_str:
            return ToolResult(
                output=("No replacement was performed. `new_str` and `old_str` must be " "different."),
                success=False,
            )
        ...
        if not occurrences:
            ...
            if not occurrences:
                return ToolResult(
                    output=(
                        f"No replacement was performed, old_str `{old_str}` did not " f"appear verbatim in {path}."
                    ),
                    success=False,
                )
        ...
        snippet = self._editor_slice_content(new_file_content, start_line + 1, end_line)

        success_message = f"The file {path} has been edited. "
        success_message += self._editor_make_output(snippet, f"a snippet of {path}", start_line + 1)
        success_message += (
    ...
    def _editor_insert(self, path: str, insert_line: Optional[int], new_str: Optional[str]) -> "ToolResult":
        ...
        if insert_line < 0 or insert_line > num_lines:
            return ToolResult(
                output=(f"It should be within the range of allowed values: " f"{[0, num_lines]}"),
                success=False,
            )
    ...
    def _editor_undo(self, path: str) -> "ToolResult":
        ...
        output = f"Last edit to {path} undone successfully. " f"{self._editor_make_output(old_text, str(path))}"
        return ToolResult(output=output, success=True)
    ...
    def write_file(self, path: str, content: str) -> ExecutionResult:
        ...
        path_quoted = shlex.quote(path)
        cmd = f"cat > {path_quoted} << 'NANOROLLOUT_EOF_31415926'\n" f"{content}\nNANOROLLOUT_EOF_31415926"
    ...
    def list_directory(self, path: str, max_depth: int = 2) -> str:
        ...
        path_quoted = shlex.quote(path)
        result = self.execute(f"find {path_quoted} -maxdepth {max_depth} -not -path '*/.*' " "2>/dev/null | head -100")
    ...
    def get_git_diff(self) -> str:
        ...
        check = self.execute(f"cd {repo_dir_quoted} && git rev-parse --is-inside-work-tree")
        ...
        self.execute(f"cd {repo_dir_quoted} && " f"git commit --no-verify -m '{GIT_COMMIT_MESSAGE}'")

        if base_commit:
            diff_result = self.execute(
                f"cd {repo_dir_quoted} && " f"git --no-pager diff --no-color {base_commit} HEAD"
            )
        ...
        fallback = self.execute(f"cd {repo_dir_quoted} && git --no-pager diff --no-color HEAD~1 HEAD")
        return fallback.output

    def undo_edit(self, path: str) -> ExecutionResult:
        """Undo the last edit to a file. Default returns error."""
        return ExecutionResult(output="Undo not supported by this environment", exit_code=1)
