...
class GCEEnvironment(BaseEnvironment):
    ...
    async def _stop_async(self) -> None:
        ...
        if stub is not None and container_id is not None:
            try:
                await stub.StopContainer(worker_pb2.StopContainerRequest(container_id=container_id))
    ...
    def _run_async(self, coro: Any) -> Any:
        ...
        raise RuntimeError(
            "GCEEnvironment provides synchronous APIs and cannot run inside " "an already running event loop."
        )
