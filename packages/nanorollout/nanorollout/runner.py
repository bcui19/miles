...
def build_metadata(
    ...
    *,
    ...
    metadata = {
        ...
        "exit_status": exit_status or (setup_error_status if agent_result is None else None),
    }
...
def _stop_environment(
    ...
    try:
        ...
        logger.exception("%s cleanup failed for %s", adapter.runner_label, request.instance_id)
...
def _fallback_reward_payload(instance_id: str, error_msg: Optional[str]) -> Dict[str, Any]:
    return {
        "instance_id": instance_id,
        "resolved": False,
...
def run_task(
    ...
    try:
        ...
        with env_logging(trial_dir, adapter.env_logger_name):
            with trial_logging(trial_dir):
                ...
                logger.info("[%s] Running %s", request.instance_id, adapter.runner_label)
        ...
        error_msg = str(exc)
        logger.exception("%s run failed for %s", adapter.runner_label, request.instance_id)
    finally:
        if env_obj:
    ...
    try:
        ...
        logger.exception("%s reward build failed for %s", adapter.runner_label, request.instance_id)
    ...
    try:
        ...
        logger.exception("%s result write failed for %s", adapter.runner_label, request.instance_id)
        metadata["error"] = error_msg
    ...
    try:
        ...
        logger.exception("%s metrics build failed for %s", adapter.runner_label, request.instance_id)
        metadata["error"] = error_msg
