...
import ray
from nanorollout.core.config import SchedulerConfig
from nanorollout.core.models import ResourceRequest, RunRequest
from nanorollout.core.runners import build_runner_params, load_runner_callable, resolve_request_runner
...
class Scheduler:
    def __init__(
        ...
        self._executor: ThreadPoolExecutor | None = ThreadPoolExecutor(max_workers=pool_size)
        ...
        if task_timeout_s is not None and task_timeout_s > 0:
            try:
                ...
                try:
                    ...
                    ray.cancel(result_ref, force=True, recursive=True)
                raise SchedulerTimeoutError(f"Task exceeded timeout: {task_timeout_s}s") from exc
        else:
            result = ray.get(result_ref)

        if isinstance(result, dict) and result.get("error"):
            raise RuntimeError(f"Ray task failed: {result.get('error')}\n{result.get('traceback', '')}")
