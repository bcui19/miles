...
def load_runner_callable(module_name: str, entrypoint: str):
    ...
    try:
        ...
        raise ImportError(f"Runner entrypoint {entrypoint!r} not found in module {module_name!r}") from exc


def build_runner_params(request: RunRequest, output_dir: str) -> dict[str, Any]:
    # Validate the task/agent pair resolves to a known runner (raises otherwise).
    resolve_request_runner(request)
