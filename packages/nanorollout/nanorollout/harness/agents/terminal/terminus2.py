...
from litellm import completion, completion_cost, get_model_info
...
class Terminus2Agent:
    ...
    def _get_completion_confirmation_message(self, terminal_output: str) -> str:
        if self._parser_name == "json":
            return COMPLETION_CONFIRMATION_JSON.format(terminal_output=terminal_output)
        return COMPLETION_CONFIRMATION_XML.format(terminal_output=terminal_output)
    ...
    @retry(
        ...
        retry=(retry_if_not_exception_type(_ContextLengthError) & retry_if_exception_type(Exception)),
        reraise=True,
    )
    def _completion_with_retry(self, **kwargs) -> Any:
        ...
        try:
            ...
            err = str(e).lower()
            if self._enable_summarize and any(
                p in err
                for p in (
                    "context length",
    ...
    def _get_model_context_limit(self) -> int:
        ...
        try:
            ...
            return int(info.get("max_input_tokens") or info.get("max_tokens") or 1_000_000)
        except Exception:
            return 1_000_000
    ...
    def _summarize(self, instruction: str, tmux: "TmuxAdapter") -> str:
        ...
        summary = self._subagent_call(list(self._messages) + [{"role": "user", "content": summary_prompt}])
        ...
        for command in commands:
            try:
                ...
                return True, TIMEOUT_TEMPLATE.format(
                    ...
                    terminal_state=self._limit_output_length(tmux.get_incremental_output()),
                )
    ...
    def run(self, instruction: str, total_timeout: Optional[float] = None) -> "Terminus2Result":
        ...
        """
        local_cast = Path(self._output_dir) / "recording.cast" if self._enable_asciinema and self._output_dir else None
        tmux = TmuxAdapter(
            self._env,
        ...
        try:
            while self._max_iterations is None or iterations < self._max_iterations:
                ...
                if self._enable_summarize:
                    handoff = self._check_proactive_summarization(original_instruction, tmux)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            short_response = self._subagent_call([{"role": "user", "content": short_prompt}])
                ...
                commands = [
                    Command(keystrokes=pc.keystrokes, duration_sec=min(pc.duration, 60)) for pc in result.commands
                ]
                timeout_occurred, terminal_output = self._execute_commands(commands, tmux)
        ...
        return Terminus2Result(
            ...
            llm_cost_total=sum(
                m.get("cost", 0.0) for m in self.llm_metrics if isinstance(m.get("cost"), (int, float))
            ),
            patch="",
