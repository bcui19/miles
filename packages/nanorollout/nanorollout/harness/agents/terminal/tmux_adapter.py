...
class TmuxAdapter:
    ...
    def start(self) -> None:
        ...
        if result.exit_code != 0:
            raise RuntimeError(f"Failed to start tmux session: {result.output}")
        ...
        if self._enable_asciinema:
            ...
            if self._local_cast_path is not None:
                ...
                if result.exit_code == 0 and result.output:
                    ...
                    logger.warning("Failed to download asciinema recording: %s", result.output)
    ...
    def _tmux_send_keys(self, keys: list[str]) -> str:
        ...
        return " ".join(["tmux", "send-keys", "-t", shlex.quote(self._session_name), *escaped])
    ...
    def capture_pane(self, capture_entire: bool = False) -> str:
        if capture_entire:
            result = self._env.execute(f"tmux capture-pane -p -S - -t {self._session_name}")
        else:
            result = self._env.execute(f"tmux capture-pane -p -t {self._session_name}")
        ...
        if pkg_manager:
            ...
            if install_cmd:
                ...
                if result.exit_code == 0:
                    ...
                    if self._enable_asciinema and not asciinema_installed:
                        if self._env.execute("asciinema --version", timeout=_VERIFY_TIMEOUT_SEC).exit_code != 0:
    ...
    def _build_tmux_from_source(self) -> None:
        ...
        if self._env.execute("tmux -V || /usr/local/bin/tmux -V", timeout=_VERIFY_TIMEOUT_SEC).exit_code == 0:
