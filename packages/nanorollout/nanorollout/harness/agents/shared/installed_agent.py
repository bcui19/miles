...
def _coerce_value(
    ...
    if type_name == "str":
        if isinstance(value, bool):
            raise ValueError(f"Invalid value for '{kwarg_name}': expected str, got bool")
        if isinstance(value, (int, float)):
            return str(value)
        if not isinstance(value, str):
            raise ValueError(f"Invalid value for '{kwarg_name}': expected str, got {type(value).__name__}")
        return value

    if type_name == "int":
        if isinstance(value, bool):
            raise ValueError(f"Invalid value for '{kwarg_name}': expected int, got bool")
        ...
        if isinstance(value, str):
            return int(value)
        raise ValueError(f"Invalid value for '{kwarg_name}': expected int, got {type(value).__name__}")
    ...
    if type_name == "enum":
        ...
        if choices and normalized not in choices:
            ...
            raise ValueError(f"Invalid value for '{kwarg_name}': {value!r}. Valid values: {valid}")
        return normalized
...
class InstalledAgentBase(ABC):
    ...
    def _resolve_flag_values(self) -> dict[str, Any]:
        ...
        for descriptor in self.CLI_FLAGS:
            ...
            if raw is None:
                continue
            resolved[descriptor.kwarg] = _coerce_value(raw, descriptor.type, descriptor.choices, descriptor.kwarg)
        return resolved
    ...
    def _resolve_env_values(self) -> dict[str, str]:
        ...
        for descriptor in self.ENV_VARS:
            ...
            if raw is None:
                continue
            coerced = _coerce_value(raw, descriptor.type, descriptor.choices, descriptor.kwarg)
            if descriptor.type == "bool":
                resolved[descriptor.env] = descriptor.bool_true if coerced else descriptor.bool_false
            else:
    ...
    def _prepare_remote_logs_dir(self, environment: ShellEnvironment) -> None:
        ...
        self._remote_logs_dir = PurePosixPath("/tmp") / (f"nanorollout-{slug}-{uuid.uuid4().hex[:8]}")
        self.exec(
            environment,
    ...
    def exec(
        ...
        *,
        ...
        if check and result.exit_code != 0:
            raise NonZeroAgentExitCodeError(f"Command failed (exit {result.exit_code}): {command}\n{result.output}")
        return result
    ...
    def _sync_remote_logs(self, environment: ShellEnvironment) -> None:
        ...
        result = self.exec(
            environment,
            (f"if [ -d {remote_dir} ]; then " f"tar -C {remote_dir} -cf - . | base64 | tr -d '\\n'; " "fi"),
    ...
    def _trajectory_to_history(self, trajectory: dict[str, Any] | None) -> list[dict[str, Any]]:
        if not trajectory:
            return []
    ...
    def _trajectory_to_llm_metrics(self, trajectory: dict[str, Any] | None) -> list[dict[str, Any]]:
        if not trajectory:
            return []
    ...
    def run(
        ...
        *,
        ...
        if llm_cost_total is None:
            llm_cost_total = sum(
                metric.get("cost", 0.0) for metric in llm_metrics if isinstance(metric.get("cost"), (int, float))
            )
