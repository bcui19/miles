...
def main():
    ...
    # Also block dangerous process-killing commands anywhere in the command
    # (catches `bash -c "pkill ..."`, pipes, etc.)
    _DANGEROUS_CMDS = {"kill", "pkill", "killall", "skill", "xkill"}
    cmd_tokens = set(args.cmd.split())
    matched = _DANGEROUS_CMDS & cmd_tokens
    if matched:
        print(
            f"Command contains blocked process-killing command(s): {matched}. "
            "Do not try to kill processes in this environment."
        )
        sys.exit(1)
