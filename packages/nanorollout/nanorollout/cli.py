...
def build_config(args: argparse.Namespace, overrides: List[str]) -> ServerConfig:
    ...
    file_cfg = OmegaConf.load(args.config) if Path(args.config).exists() else OmegaConf.create({})
...
def _parse_env_assignments(values: list[str]) -> dict[str, str]:
    ...
    for value in values:
        if "=" not in value:
            raise ValueError(f"Invalid --agent-env value {value!r}; expected KEY=VALUE")
...
def _build_run_requests(args: argparse.Namespace) -> list[RunRequest]:
    ...
    if args.request_file:
        requests.extend(_row_to_request(row, args) for row in _load_request_rows(args.request_file))
    else:
