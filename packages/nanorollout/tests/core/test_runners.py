...
from nanorollout.adapters.terminal.adapter import TerminalTaskAdapter, TerminalTaskSpec
...
def test_swe_and_terminal_specs_use_consolidated_modules() -> None:
    assert resolve_runner("swe", "oh-core").module == "nanorollout.adapters.swe.entrypoints"
    assert resolve_runner("swe", "claudecode").entrypoint == "run_installed_claude_code"
    assert resolve_runner("terminal", "terminus-2").module == "nanorollout.adapters.terminal.entrypoints"
